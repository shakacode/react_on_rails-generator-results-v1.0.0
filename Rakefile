...
task "pull_all", [:version] do |_task, args|
  RESULT_TYPES.each do |result_type|
    ...
    sh %(git checkout #{branch_name})
    message = branch_name
    sh %(hub pull-request -b master -h shakacode:#{branch_name} -m "#{message}")
  end
  ...
  COMPARISON_TYPES.each do |comparison_type|
    ...
    message = "#{comparison_type[0]} -> #{comparison_type[1]} comparison v#{args[:version]}"
    sh %(hub pull-request -m "#{message}" -b shakacode:#{base_branch_name} -h shakacode:#{head_branch_name})
  end
end
